 ...
 *
 * Один DOM, адаптив через `xl:`-варианты (Figma 334:302 desktop / 2324:2839 mobile):
 ...
 *   • ≥ xl  — flex-row justify-between items-center: слева текст-колонка
...
<section class="bg-mrent-black px-3.75 py-15 xl:px-25 xl:py-25">
	<div class="max-w-430 mx-auto flex flex-col gap-7.5 xl:flex-row xl:items-center xl:justify-between xl:gap-10">
		...
		<?php /* Текстовая колонка. На мобайле — gap-30 между текст-блоком и ссылкой,
		         внутри текст-блока gap-15. На ≥xl — gap-50 между текст-блоком и
		         ссылкой, внутри текст-блока gap-30 (Figma 329:294/295). */ ?>
		<div class="flex flex-col gap-7.5 xl:gap-12.5 xl:shrink-0">
			<div class="flex flex-col gap-3.75 xl:gap-7.5 text-mrent-white leading-[1.2]">
				...
				<?php if ($mrent_lead) : ?>
					<p class="text-[clamp(14px,calc(12.6px+0.91vw),20px)]">
		...
		<?php /* Контейнер карточек.
		         • Mobile (<xl): w-full с aspect-[330/214] — занимает всю ширину
		         ...
		         • ≥xl: фикс 801×398 c пиксельными офсетами 371/141 и карточками
		           430×257 (Figma 334:302), `aspect-auto` сбрасывает мобильное
		           aspect-ratio. DOM-порядок gold → silver — silver сверху
		           в зоне пересечения. */ ?>
		<div class="relative w-full aspect-[330/214] xl:flex-1 xl:min-w-0 xl:max-w-[801px] xl:aspect-[801/398]">
			<img
				src="<?php echo esc_url($mrent_image_1_url); ?>"
				alt="<?php echo esc_attr($mrent_image_1_alt); ?>"
				class="absolute top-0 left-1/3 block w-2/3 aspect-[220/131] object-contain xl:left-[46.317%] xl:w-[53.683%] xl:aspect-[430/257]"
				loading="lazy"
				decoding="async">
			<img
				src="<?php echo esc_url($mrent_image_2_url); ?>"
				alt="<?php echo esc_attr($mrent_image_2_alt); ?>"
				class="absolute top-[38.785%] left-0 block w-2/3 aspect-[220/131] object-contain xl:top-[35.427%] xl:w-[53.683%] xl:aspect-[430/257]"
				loading="lazy"
				decoding="async">
		</div>
