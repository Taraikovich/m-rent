...
<section class="2xl:hidden relative bg-mrent-black overflow-hidden">
	...
	<div class="relative z-10 pt-[72px] px-[15px] flex flex-wrap gap-[6px] items-center justify-center">
		...
		?>
			<a href="#category-<?php echo esc_attr( $slug ); ?>" class="bg-[#252426] hover:bg-[#2f2e30] transition-colors rounded-[15px] px-[15px] py-[12px] flex items-center justify-center text-white text-[14px] leading-[1.2] whitespace-nowrap">
				<?php echo esc_html( $cat['category_label'] ); ?>
			</a>
		<?php endforeach; ?>
	</div>

	<?php /* Hero-блок: фото на фоне с двойным градиентом, заголовок и CTA в потоке.
	         Большой pt сверху отдаёт место под видимую часть авто над текстом. */ ?>
	<div class="relative mt-[12px]">
		<div class="absolute inset-0 overflow-hidden">
			<img src="<?php echo esc_url( $mrent_hero_url ); ?>" alt="<?php echo esc_attr( $mrent_hero_alt ); ?>" class="absolute inset-0 size-full object-cover" loading="eager" decoding="async" fetchpriority="high">
			<div class="absolute inset-0 mrent-hero-overlay-mobile"></div>
		</div>
		<div class="relative pt-[290px] pb-[30px] px-[15px] flex flex-col gap-[30px] items-center text-white">
...
<section class="hidden 2xl:block relative bg-mrent-black">
	...
	<div class="relative h-[802px]">
		<div class="absolute inset-0 overflow-hidden">
			<img src="<?php echo esc_url( $mrent_hero_url ); ?>" alt="<?php echo esc_attr( $mrent_hero_alt ); ?>" class="absolute inset-0 size-full object-cover object-[center_70%]" loading="eager" decoding="async" fetchpriority="high">
