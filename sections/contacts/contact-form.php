 ...
 *
 * Раскладка:
 *   Desktop (xl+, max-w-1720): 2 колонки flex-row gap-70 items-center.
 *     Левая (w-805 flex-col justify-between):
 *       Верх — title-блок (gap-22): H1 «Контакты» 74px Bold + лид 30px Book (w-545),
 *       затем Contact Info: 2 колонки gap-50 (телефон+email+компания / адрес+часы),
 *       каждая ячейка — icon 30×30 + текст 24px Book.
 *       Низ — ряд соц-иконок 70×70 gap-14.
 *     Правая: CF7-форма w-845 в `.mrent-consultation-form`.
 *   Mobile (<xl): 1 колонка gap-30. H1 28px + лид 16px, контакты строкой
...
$mrent_company  = __( 'Название компании', 'm-rent' );
$mrent_unp      = __( 'УНП 123456789', 'm-rent' );
$mrent_addr_1   = __( 'г. Минск, ул.Название, 1', 'm-rent' );
$mrent_addr_2   = __( 'г. Брест, ул.Название, 1', 'm-rent' );
	...
	<div class="max-w-[1720px] mx-auto flex flex-col xl:flex-row xl:items-stretch gap-[30px] xl:gap-[70px]">
		...
		<div class="flex flex-col xl:w-[805px] xl:shrink-0 xl:justify-between gap-[30px] xl:gap-0">
			...
			<div class="flex flex-col gap-[30px] xl:gap-[80px]">
				...
				<?php /* Заголовок + лид. */ ?>
				<div class="flex flex-col gap-[15px] xl:gap-[22px] text-mrent-white leading-[1.2]">
					<h1 class="font-display font-bold text-[28px] xl:text-[74px]">
						<?php esc_html_e( 'Контакты', 'm-rent' ); ?>
					</h1>
					<p class="font-display text-[16px] xl:text-[30px] xl:max-w-[545px]">
					...
					<div class="flex flex-col gap-[10px] xl:gap-[20px]">
						...
						<?php if ( $mrent_phone !== '' ) : ?>
							<a href="<?php echo esc_url( $mrent_phone_url ); ?>" class="flex items-center gap-[15px] group">
								...
								<span class="font-display text-[14px] xl:text-[24px] text-mrent-white leading-[1.2] whitespace-nowrap group-hover:opacity-80 transition-opacity">
						...
						<div class="flex items-center gap-[15px]">
							...
							<span class="font-display text-[14px] xl:text-[24px] text-mrent-white leading-[1.2] whitespace-nowrap">
			...
			<?php if ( $mrent_brand_links ) : ?>
				<div class="flex items-center gap-[10px] xl:gap-[14px] xl:mt-0 mt-[10px]">
					<?php foreach ( $mrent_brand_links as $mrent_slug => $mrent_url ) : ?>
						...
						<a
							...
							class="block size-[38px] xl:size-[70px] hover:opacity-80 transition-opacity shrink-0"
