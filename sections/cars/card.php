 ...
 *
 * Десктоп (390:386): ширина 408px, фото 600px, подпись 26px, цена 30px,
 *                   две кнопки в ряд по 60px.
 * Мобайл  (2326:3641): ширина 160px, фото 224px, цена 16px, без подписи,
...
<article class="flex flex-col gap-[15px] xl:gap-[30px] items-stretch w-full h-full">
	<a href="<?php echo esc_url( $mrent_permalink ); ?>" class="block relative rounded-[15px] overflow-hidden h-[224px] xl:h-[600px] bg-[#252426] shrink-0">
	...
	<div class="flex flex-col gap-[15px] xl:gap-[30px] items-start w-full flex-1">
		<?php /* На мобайле название скрыто — по дизайну 2326:3641 у карточки только цена.
		         На xl+ показываем заголовок шрифтом 26px Bold (Figma 390:386). */ ?>
		<h3 class="hidden xl:block w-full font-[700] text-[26px] leading-[1.2] text-mrent-white">
			<a href="<?php echo esc_url( $mrent_permalink ); ?>" class="hover:text-mrent-yellow transition-colors">
				<?php the_title(); ?>
			</a>
		</h3>

		<?php /* Прайс + кнопки — единый блок, прилипший к низу карточки на xl+.
		...
		<div class="flex flex-col gap-[15px] xl:gap-[30px] items-start w-full xl:mt-auto">
			...
			<div class="flex flex-col xl:flex-row gap-[10px] items-stretch w-full">
				<a href="<?php echo esc_url( $mrent_booking_url ); ?>" class="bg-mrent-yellow hover:bg-[#FFF831] w-full xl:w-auto xl:flex-1 shrink-0 h-[55px] xl:h-[60px] flex items-center justify-center rounded-[15px] text-mrent-black font-[500] text-[14px] xl:text-[20px] whitespace-nowrap transition-colors">
					<?php esc_html_e( 'Забронировать', 'm-rent' ); ?>
				</a>
				<a href="<?php echo esc_url( $mrent_permalink ); ?>" class="border border-mrent-white hover:bg-mrent-white/10 w-full xl:w-auto xl:flex-1 shrink-0 h-[55px] xl:h-[60px] flex items-center justify-center rounded-[15px] text-mrent-white font-[500] text-[14px] xl:text-[20px] whitespace-nowrap transition-colors">
