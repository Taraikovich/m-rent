...
<section class="bg-mrent-black pt-[30px] xl:pt-[50px] pb-[60px] xl:pb-[100px]">
	<div class="px-[15px] xl:px-[100px]">
		<div class="max-w-[1720px] mx-auto">
		<?php if ( have_posts() ) : ?>
			<div class="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-[10px] xl:gap-[29px]">
			...
			if ( $mrent_pagination ) : ?>
				<nav class="mt-[40px] xl:mt-[60px] flex flex-wrap gap-[10px] justify-center text-mrent-white">
					<?php foreach ( $mrent_pagination as $link ) : ?>
						<span class="[&_a]:px-3 [&_a]:py-2 [&_a]:rounded-md [&_a]:bg-[#252426] [&_a:hover]:bg-[#2f2e30] [&_.current]:px-3 [&_.current]:py-2 [&_.current]:rounded-md [&_.current]:bg-mrent-yellow [&_.current]:text-mrent-black"><?php echo $link; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped — paginate_links уже экранирует ?></span>
					<?php endforeach; ?>
				</nav>
			<?php endif; ?>
		<?php else : ?>
